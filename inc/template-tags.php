...
if ( ! function_exists( 'custom_theme_posted_by' ) ) :
	...
	function custom_theme_posted_by() {
		$byline = sprintf(
			...
			esc_html_x( 'by %s', 'post author', 'custom_theme' ),
	...
	function custom_theme_entry_footer() {
		...
		if ( 'post' === get_post_type() ) {
			...
			if ( $categories_list ) {
				...
				printf( '<span class="cat-links">' . esc_html__( 'Posted in %1$s', 'custom_theme' ) . '</span>', $categories_list ); // WPCS: XSS OK.
			}

			/* translators: used between list items, there is a space after the comma */
			$tags_list = get_the_tag_list( '', esc_html_x( ', ', 'list item separator', 'custom_theme' ) );
			if ( $tags_list ) {
				/* translators: 1: list of tags. */
				printf( '<span class="tags-links">' . esc_html__( 'Tagged %1$s', 'custom_theme' ) . '</span>', $tags_list ); // WPCS: XSS OK.
			}
		}
...
if ( ! function_exists( 'custom_theme_post_thumbnail' ) ) :
	...
	function custom_theme_post_thumbnail() {
		...
		if ( is_singular() ) :
			...
			<div class="entry-beer-thumbnail<?php if ( get_field( 'beer_new' ) ): ?> entry-beer-thumbnail-new<?php endif; ?> mt-2">
				<?php the_post_thumbnail( 'post-thumbnail', [ 'class' => 'img-fluid rounded' ] ); ?>
			</div><!-- .post-thumbnail -->

		<?php else : ?>

		<a class="entry-beer-thumbnail" href="<?php the_permalink(); ?>" aria-hidden="true" tabindex="-1">
			<?php
